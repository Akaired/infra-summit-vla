"""Run the scripted MuJoCo episode from a live Speechmatics voice command.
...
# Keep this isolated so replacing the scripted episode with the VLA runner is
# a one-line change: [sys.executable, "run_vla.py", "0"].
EPISODE_COMMAND: Final = (sys.executable, "test_phases.py", "0")
...
async def run_episode() -> int:
    """Run the selected episode and print its complete captured result."""
    print(f"Trigger matched. Launching: {' '.join(EPISODE_COMMAND)}", flush=True)
    ...
    print(f"Episode exit code: {process.returncode}")
    if stdout:
        print("--- episode stdout ---")
        print(stdout.decode("utf-8", errors="replace"), end="")
    if stderr:
        print("--- episode stderr ---", file=sys.stderr)
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        async with session.ws_connect(url, headers=headers, heartbeat=20) as ws:
            ...
            try:
                ...
                # The final transcript task returns first. Let the microphone
                # task observe the event and release the audio device cleanly.
                await microphone
            finally:
